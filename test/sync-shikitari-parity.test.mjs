...
test("Shikitari receives the same managed repository metadata as Wabachi", () => {
  assert.deepEqual(
    normalizedMappings(sync, "yohn-jp/shikitari"),
    normalizedMappings(sync, "yohn-jp/wabachi")
  );
});

test("Shikitari receives the same shared agent governance as Wabachi", () => {
  assert.deepEqual(
    normalizedMappings(agentSync, "yohn-jp/shikitari"),
    normalizedMappings(agentSync, "yohn-jp/wabachi")
  );
});

test("Shikitari stays in the template-sync GitHub App repository scope", () => {
  ...
  assert.ok(
    repositories.includes("shikitari"),
    "sync-org-templates.yml must request an installation token for shikitari"
  );
});
