...
test("unquoted-comma flow-mapping regression is caught", () => {
  const errors = validateIssueFormFile(
    "test/fixtures/issue-forms/invalid-flow-mapping.yml"
  );
  assert.ok(errors.length > 0, "expected validation errors");
  assert.ok(
    errors.some(
      (e) => e.includes('"observed result"') || e.includes('"description"')
    ),
...
test("unsupported element type is rejected", () => {
  const errors = validateIssueFormFile(
    "test/fixtures/issue-forms/invalid-unknown-type.yml"
  );
...
test("missing required top-level and attribute keys are rejected", () => {
  const errors = validateIssueFormFile(
    "test/fixtures/issue-forms/invalid-missing-required.yml"
  );
  assert.ok(
    errors.some((e) => e.includes('missing required top-level key "name"'))
  );
